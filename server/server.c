...
void serve_index(int client_socket, char *pageName)
{
    ...
    html[sizeof(html) - 1] =
        '\0'; // Assurez-vous que le buffer est null-terminé
    fclose(fp);
...
void handle_upload(int client_socket, const char *body)
{
    ...
    const char *text =
        body + strlen("text="); // avoir le pointeur vers le text sans 'text='
...
void handle_request(int client_socket)
{
    ...
    if (strstr(buffer, "GET / ") != NULL)
    {
        serve_index(client_socket,
                    "server/source/index.html"); // envoie la racine
        return;
    }
    ...
    {
        serve_index(client_socket,
                    "server/source/upload.html"); // envoie le lien pour
                                                  // rediriger a la racine
        return;
    }
